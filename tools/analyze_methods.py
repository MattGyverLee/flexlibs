...
def main():
    ...
    already_delegated = {
        ...
        "GetFeatures",
        # WS 7
        "GetAllWritingSystems",
        "GetVernacularWritingSystems",
        "GetAnalysisWritingSystems",
        "GetWritingSystemName",
        "GetWritingSystemAbbreviation",
        "IsVernacularWS",
        "IsAnalysisWS",
        # CustomField 9
        "GetAllCustomFields",
        ...
        "GetCustomFieldChoices",
        "AddCustomFieldChoice",
    }
    ...
    for method in methods:
        ...
        if (
            name.startswith("GetAllVernacular")
            or name.startswith("GetAllAnalysis")
            or name.startswith("GetDefault")
            or name == "WSUIName"
            or name == "WSHandle"
            or name == "GetWritingSystems"
        ):
            category_a.append(
                {"name": name, "line": line, "ops_class": "WritingSystemOperations", "note": "WS utility method"}
            )
            continue
        ...
        if name.startswith("Lexicon"):
            ...
            category_a.append(
                {"name": name, "line": line, "ops_class": ops_class, "note": "Lexicon method - likely delegatable"}
            )
            continue
        ...
        if name.startswith("ObjectsIn") or name.startswith("ObjectCountFor"):
            category_a.append(
                {"name": name, "line": line, "ops_class": "Repository", "note": "Repository query method"}
            )
            continue
        ...
        if name.startswith("Texts"):
            category_a.append(
                {"name": name, "line": line, "ops_class": "TextOperations", "note": "Text-specific method"}
            )
            continue

        if name.startswith("Reversal"):
            category_a.append(
                {"name": name, "line": line, "ops_class": "ReversalOperations", "note": "Reversal-specific method"}
            )
            continue
        ...
        if (
            name.startswith("GetLexicalRelationTypes")
            or name.startswith("GetPublications")
            or name.startswith("PublicationType")
        ):
            category_a.append(
                {
                    "name": name,
        ...
        if name.startswith("GetFieldID"):
            category_a.append(
                {"name": name, "line": line, "ops_class": "CustomFieldOperations", "note": "Field metadata method"}
            )
            continue
    ...
    total_accounted = len(already_delegated) + len(category_a) + len(category_b) + len(category_c)
