...
def demo_morphrule_crud():
    ...
    try:
        ...
        initial_count = 0
        for obj in project.Morphrule.GetAll():
            # Display first few objects
            try:
                name = project.Morphrule.GetName(obj) if hasattr(project.Morphrule, 'GetName') else str(obj)
                print(f"  - {name}")
            except:
        ...
        try:
            if hasattr(project.Morphrule, 'Exists') and project.Morphrule.Exists(test_name):
                print(f"\nTest morphrule '{test_name}' already exists")
                print("Deleting existing one first...")
                existing = project.Morphrule.Find(test_name) if hasattr(project.Morphrule, 'Find') else None
                if existing:
                    project.Morphrule.Delete(existing)
        ...
        try:
            ...
            test_obj = project.Morphrule.Create(test_name)
        except TypeError:
            try:
                # Try without parameters if that fails
                test_obj = project.Morphrule.Create()
                if hasattr(project.Morphrule, 'SetName'):
                    project.Morphrule.SetName(test_obj, test_name)
            except Exception as e:
        ...
        if test_obj:
            ...
            try:
                if hasattr(project.Morphrule, 'GetName'):
                    print(f"  Name: {project.Morphrule.GetName(test_obj)}")
            except:
                pass
        else:
        ...
        # Test Exists
        if hasattr(project.Morphrule, 'Exists'):
            print(f"\nChecking if '{test_name}' exists...")
            exists = project.Morphrule.Exists(test_name)
            print(f"  Exists: {exists}")

        # Test Find
        if hasattr(project.Morphrule, 'Find'):
            print(f"\nFinding morphrule by name...")
            found_obj = project.Morphrule.Find(test_name)
            if found_obj:
                print(f"  FOUND: morphrule")
                try:
                    if hasattr(project.Morphrule, 'GetName'):
                        print(f"  Name: {project.Morphrule.GetName(found_obj)}")
                except:
                    pass
            else:
                print("  NOT FOUND")
        ...
        current_count = sum(1 for _ in project.Morphrule.GetAll())
        ...
        if test_obj:
            ...
            if hasattr(project.Morphrule, 'SetName'):
                try:
                    ...
                    old_name = project.Morphrule.GetName(test_obj) if hasattr(project.Morphrule, 'GetName') else test_name
                    project.Morphrule.SetName(test_obj, new_name)
                    updated_name = project.Morphrule.GetName(test_obj) if hasattr(project.Morphrule, 'GetName') else new_name
            ...
            for method_name in dir(project.Morphrule):
        ...
        if hasattr(project.Morphrule, 'Find'):
            ...
            updated_obj = project.Morphrule.Find(test_name)
            if updated_obj:
                print(f"  FOUND: morphrule")
                try:
                    if hasattr(project.Morphrule, 'GetName'):
                        print(f"  Name: {project.Morphrule.GetName(updated_obj)}")
                except:
                    pass
            else:
        ...
        if test_obj:
            ...
            try:
                obj_name = project.Morphrule.GetName(test_obj) if hasattr(project.Morphrule, 'GetName') else test_name
            except:
                obj_name = test_name

            project.Morphrule.Delete(test_obj)
            print(f"  Deleted: {obj_name}")

            # Verify deletion
            print("\nVerifying deletion...")
            if hasattr(project.Morphrule, 'Exists'):
                still_exists = project.Morphrule.Exists(test_name)
            ...
            final_count = sum(1 for _ in project.Morphrule.GetAll())
        ...
        try:
            for name in ["crud_test_morphrule", "crud_test_morphrule_modified"]:
                if hasattr(project.Morphrule, 'Exists') and project.Morphrule.Exists(name):
                    obj = project.Morphrule.Find(name) if hasattr(project.Morphrule, 'Find') else None
                    if obj:
                        project.Morphrule.Delete(obj)
