...
def demo_naturalclass_crud():
    ...
    try:
        ...
        initial_count = 0
        for obj in project.Naturalclass.GetAll():
            # Display first few objects
            try:
                name = project.Naturalclass.GetName(obj) if hasattr(project.Naturalclass, 'GetName') else str(obj)
                print(f"  - {name}")
            except:
        ...
        try:
            if hasattr(project.Naturalclass, 'Exists') and project.Naturalclass.Exists(test_name):
                print(f"\nTest naturalclass '{test_name}' already exists")
                print("Deleting existing one first...")
                existing = project.Naturalclass.Find(test_name) if hasattr(project.Naturalclass, 'Find') else None
                if existing:
                    project.Naturalclass.Delete(existing)
        ...
        try:
            ...
            test_obj = project.Naturalclass.Create(test_name)
        except TypeError:
            try:
                # Try without parameters if that fails
                test_obj = project.Naturalclass.Create()
                if hasattr(project.Naturalclass, 'SetName'):
                    project.Naturalclass.SetName(test_obj, test_name)
            except Exception as e:
        ...
        if test_obj:
            ...
            try:
                if hasattr(project.Naturalclass, 'GetName'):
                    print(f"  Name: {project.Naturalclass.GetName(test_obj)}")
            except:
                pass
        else:
        ...
        # Test Exists
        if hasattr(project.Naturalclass, 'Exists'):
            print(f"\nChecking if '{test_name}' exists...")
            exists = project.Naturalclass.Exists(test_name)
            print(f"  Exists: {exists}")

        # Test Find
        if hasattr(project.Naturalclass, 'Find'):
            print(f"\nFinding naturalclass by name...")
            found_obj = project.Naturalclass.Find(test_name)
            if found_obj:
                print(f"  FOUND: naturalclass")
                try:
                    if hasattr(project.Naturalclass, 'GetName'):
                        print(f"  Name: {project.Naturalclass.GetName(found_obj)}")
                except:
                    pass
            else:
                print("  NOT FOUND")
        ...
        current_count = sum(1 for _ in project.Naturalclass.GetAll())
        ...
        if test_obj:
            ...
            if hasattr(project.Naturalclass, 'SetName'):
                try:
                    ...
                    old_name = project.Naturalclass.GetName(test_obj) if hasattr(project.Naturalclass, 'GetName') else test_name
                    project.Naturalclass.SetName(test_obj, new_name)
                    updated_name = project.Naturalclass.GetName(test_obj) if hasattr(project.Naturalclass, 'GetName') else new_name
            ...
            for method_name in dir(project.Naturalclass):
        ...
        if hasattr(project.Naturalclass, 'Find'):
            ...
            updated_obj = project.Naturalclass.Find(test_name)
            if updated_obj:
                print(f"  FOUND: naturalclass")
                try:
                    if hasattr(project.Naturalclass, 'GetName'):
                        print(f"  Name: {project.Naturalclass.GetName(updated_obj)}")
                except:
                    pass
            else:
        ...
        if test_obj:
            ...
            try:
                obj_name = project.Naturalclass.GetName(test_obj) if hasattr(project.Naturalclass, 'GetName') else test_name
            except:
                obj_name = test_name

            project.Naturalclass.Delete(test_obj)
            print(f"  Deleted: {obj_name}")

            # Verify deletion
            print("\nVerifying deletion...")
            if hasattr(project.Naturalclass, 'Exists'):
                still_exists = project.Naturalclass.Exists(test_name)
            ...
            final_count = sum(1 for _ in project.Naturalclass.GetAll())
        ...
        try:
            for name in ["crud_test_naturalclass", "crud_test_naturalclass_modified"]:
                if hasattr(project.Naturalclass, 'Exists') and project.Naturalclass.Exists(name):
                    obj = project.Naturalclass.Find(name) if hasattr(project.Naturalclass, 'Find') else None
                    if obj:
                        project.Naturalclass.Delete(obj)
